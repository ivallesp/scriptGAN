...
def build_discriminator(input_, reuse=False):
    with tf.variable_scope("Discriminator", reuse=reuse):
        x = tf.layers.conv1d(input_, filters=64, kernel_size=15, padding="same", strides=1, activation=leaky_relu,
                             kernel_initializer=tf.contrib.layers.xavier_initializer(), name="conv1d_1_1")

        x = tf.layers.conv1d(x, filters=64, kernel_size=15, padding="same", strides=1, activation=leaky_relu,
        ...
        x = tf.layers.conv1d(x, filters=128, kernel_size=15, padding="same", strides=1, activation=leaky_relu,
